...
def click_proceed():
    ...
    if city_name == '' or city_name.isdigit():
        ...
        cur = mydb.cursor()
        cur.execute('Select Exists(select * from hotel_management where City_Name=%s)', (city_name,))
        result = cur.fetchall()
        avail = 0
        for i in result:
            a = list(i)
        ...
        if avail == 1 :
            mydb = mysql.connector.connect(host='localhost', user='root', password='abc456', database='hotel')
            cur = mydb.cursor()
            cur.execute('select * from hotel_management where City_Name=%s', (city_name,))
            res = cur.fetchall()
            print(res)
            avail = 0
            cur.execute('SELECT Cust_Id,Room_No,First_Name,Last_Name,Room_Type,No_Days,Checked_In,Room_Rate,Room_Desc from hotel_management\
             where City_Name =%s', (city_name,))
            result = cur.fetchall()
            ...
            text.config(state=NORMAL)
            text.delete(1.0,END)
            text.insert(INSERT, "Their is No Customer From that City!")
            city_entry.delete(0,"end")
            text.config(state=DISABLED)
...
# heading
heading_label = Label(root, text="---------  CUSTOMERS DETAILS via CITY NAME ---------",  bg="deep sky blue", fg="white", font=('Times New Roman', 15,'bold'))
...
# Room Number
city_label = Label(topFrame, text="City Name : ",font=('Times New Roman', 20,"bold"))
city_entry = Entry(topFrame, textvar=City_Name, bd=5, bg="#ccefff", fg='blue', width=15, font=('Arial', 15))

city_label.grid(row=1, column=0, padx=15, pady=10, sticky=E)
city_entry.grid(row=1, column=1, ipady=5, ipadx=60, sticky=W)
